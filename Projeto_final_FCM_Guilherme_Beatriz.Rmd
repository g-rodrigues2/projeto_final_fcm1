...
Dos mais de 64 mil pacientes, 18 pacientes se identificaram com outro gênero e nenhum deles apresentou alguma das três doenças analisadas neste primeiro momento. Portanto, os excluímos desta primeira análise. Segue que
...
Para o gênero feninino temos
```{r}
dados_dicotomicos_female_correlation <- dados_dicotomicos_female %>% 
  select_if(is.numeric) %>% 
  filter(complete.cases(.)) %>% 
  cor()
(dados_dicotomicos_female_correlation)
```
E, para o gênero masculino,
```{r}
dados_dicotomicos_male_correlation <- dados_dicotomicos_male %>% 
  select_if(is.numeric) %>% 
  filter(complete.cases(.)) %>% 
  cor()
(dados_dicotomicos_male_correlation)
```
